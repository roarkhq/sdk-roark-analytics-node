...
export class SimulationCustomerFlow extends APIResource {
   ...
   * ```ts
   * const response =
   *   await client.simulationCustomerFlow.replaceSteps(
   *     '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e',
  ...
  replaceSteps(
    flowID: string,
    body: SimulationCustomerFlowReplaceStepsParams,
    options?: RequestOptions,
  ): APIPromise<SimulationCustomerFlowReplaceStepsResponse> {
    return this._client.put(path`/v1/simulation/customer-flow/${flowID}/steps`, { body, ...options });
  }
}
...
export interface SimulationCustomerFlowReplaceStepsResponse {
  data: SimulationCustomerFlowReplaceStepsResponse.Data;
}

export namespace SimulationCustomerFlowReplaceStepsResponse {
  export interface Data {
    steps: Array<SimulationCustomerFlowAPI.FlowStep>;
...
export interface SimulationCustomerFlowReplaceStepsParams {
  /**
   * The complete set of steps. This replaces the flow's existing steps rather than
   * merging into them.
   */
  steps: Array<FlowStep>;
