...
# The entire script is one function, invoked on the last line.
#
# `sh` executes a piped script as it reads it, so under `curl ... | sh` a
# download that dies halfway through runs whatever arrived: half an installer,
# silently, exiting 0. Wrapping the body in a function means a truncated copy
# defines it and never reaches the call, so nothing happens at all - which is
# the correct outcome for a partial download. This is why rustup, Homebrew,
# Deno and nvm all do the same thing.
#
# Keep the closing `}` and the invocation as the last two meaningful lines.
__roark_install() {
  set -eu
