...
const EMBEDDED_METHODS: MethodEntry[] = [
  ...
  },
  {
    ...
    description:
      'Fetch all call-level metrics for a specific call, including both system-generated and custom metrics. By default returns only successfully computed metrics; pass `?status=all` to also include rows that resolved as NOT_APPLICABLE / DATA_MISSING / ERROR (the `value` field is omitted on those entries — check `captureStatus`).',
