...
const EMBEDDED_METHODS: MethodEntry[] = [
  ...
  },
  {
    ...
    markdown:
      '## start\n\n`client.simulationRunPlanJob.start(planId: object, flowVariables?: { flowId: string; variables: object; variantId?: string; }[], variables?: object | { scenarioId: string; variables: object; }[]): { data: object; }`\n\n**post** `/v1/simulation/plan/{planId}/job`\n\nDeprecated: use POST /v1/simulation/run, which does the same thing and can also take the\nplan configuration inline, so a one-off run does not have to create a plan first.\n\nCreates and executes a job for an existing simulation run plan. Optionally provide runtime\nvariables to override plan-defined variables.\n\n### Parameters\n\n- `planId: object`\n\n- `flowVariables?: { flowId: string; variables: object; variantId?: string; }[]`\n  Runtime variable overrides targeted at the plan’s customer flows, taking precedence over the values\npinned on the flow attachment.\n\nAn entry without `variantId` applies to every variant the attachment resolves. A flow that is not\nattached to this plan, or a variant that does not belong to the flow, is rejected rather than ignored.\n\n- `variables?: object | { scenarioId: string; variables: object; }[]`\n  Runtime variables that override the values defined on the plan. Accepts one of two formats:\n\nOption 1, global (a flat key-value object):\n  { "orderNumber": "12345", "environment": "staging" }\n\nOption 2, per-scenario (an array of objects with scenarioId + variables):\n  [\n    { "scenarioId": "550e8400-...", "variables": { "orderNumber": "12345" } },\n    { "scenarioId": "7a3d2e1f-...", "variables": { "orderNumber": "67890" } }\n  ]\n\nOn a flow-based plan the global format applies to every variant the run resolves. The per-scenario\nformat targets scenarios, so use `flowVariables` to override a specific flow or variant instead.\n\n### Returns\n\n- `{ data: { createdAt: string; simulationRunPlanId: string; simulationRunPlanJobId: string; status: string; }; }`\n\n  - `data: { createdAt: string; simulationRunPlanId: string; simulationRunPlanJobId: string; status: string; }`\n\n### Example\n\n```typescript\nimport Roark from \'@roarkanalytics/sdk\';\n\nconst client = new Roark();\n\nconst response = await client.simulationRunPlanJob.start(\'7f3e4d2c-8a91-4b5c-9e6f-1a2b3c4d5e6f\');\n\nconsole.log(response);\n```',
    perLanguage: {
      typescript: {
