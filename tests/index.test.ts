...
import { APIPromise } from 'roark-analytics/api-promise';

import util from 'node:util';
import Petstore from 'roark-analytics';
import { APIUserAbortError } from 'roark-analytics';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    test('debug logs when log level is debug', async () => {
      ...
      const client = new Petstore({ logger: logger, logLevel: 'debug', apiKey: 'My API Key' });
    ...
    test('debug logs are skipped when log level is info', async () => {
      ...
      const client = new Petstore({ logger: logger, logLevel: 'info', apiKey: 'My API Key' });
    ...
    test('client log level overrides env var', async () => {
      ...
      process.env['PETSTORE_LOG'] = 'debug';
      const client = new Petstore({ logger: logger, logLevel: 'off', apiKey: 'My API Key' });
